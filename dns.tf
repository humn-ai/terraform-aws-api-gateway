...
resource "aws_route53_record" "dns" {
  ...
  alias {
    ...
    name                   = var.endpoint_type == "EDGE" ? aws_api_gateway_domain_name.dns[each.value].cloudfront_domain_name : aws_api_gateway_domain_name.dns[each.value].domain_name
    zone_id                = var.endpoint_type == "EDGE" ? aws_api_gateway_domain_name.dns[each.value].cloudfront_zone_id : aws_api_gateway_domain_name.dns[each.value].zone_id
  }
}
